...
void current_init() {
  current_prevM = millis();
  for ( char k = 0; k < current_sensors; k++) {
    pinMode(current_pins[k], INPUT);
    pinMode(relay_pins[k], OUTPUT);
    digitalWrite(relay_pins[k], RELAY_OFF);
  }
}


void turn_on(){
  digitalWrite(relay_pins[local_id], RELAY_ON);
}

void turn_off(){
  digitalWrite(relay_pins[local_id], RELAY_OFF);
}
