...
class MACLayer{
 public:
   ...
   *          uint16_t length:           the size of the data (in uint8_t)
   *
   *  ____Out: MACTransmissionStatus: return the status of the MACLayer defined at the beginning of this class
   */
    virtual MACTransmissionStatus send(const uint8_t* lladdr_dest, uint8_t* data, uint16_t length) = 0;
  //********************************************************************************************************//
     ...
     *
     *          uint16_t* length: The pointed address will be filled with the length (in uint8_ts) of the new data.
     *
     *____Out: boolean : true--  packet for myself, and store the dst and src MAC@
     *             false-- packet for someone else
     */
    virtual bool receive(uint8_t* lladdr_src, uint8_t* lladdr_dest, uint8_t* data, uint16_t* length) = 0;
  //********************************************************************************************************//
